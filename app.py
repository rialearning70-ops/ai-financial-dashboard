...
def parse_bank_statement(uploaded_file) -> pd.DataFrame:
    ...
    try:
        ...
        if "debit" in df.columns and "credit" in df.columns:
            df["debit"] = pd.to_numeric(df["debit"].astype(str).str.replace(",", "").str.strip(), errors="coerce").fillna(0)
            df["credit"] = pd.to_numeric(df["credit"].astype(str).str.replace(",", "").str.strip(), errors="coerce").fillna(0)
            df["amount"] = df["debit"]  # Expenses = debits
...
with st.sidebar:
    ...
    if data_source == " Upload CSV":
        uploaded_file = st.file_uploader("Upload Bank Statement", type=["csv"], label_visibility="collapsed")
        st.caption("Supports HDFC, SBI, ICICI, Axis, Kotak CSV formats")
